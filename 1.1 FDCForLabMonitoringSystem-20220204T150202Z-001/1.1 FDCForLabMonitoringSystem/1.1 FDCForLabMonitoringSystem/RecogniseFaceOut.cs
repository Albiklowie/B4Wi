...
namespace _1._1_FDCForLabMonitoringSystem
{
    ...
    public partial class RecogniseFaceOut : Form
    {
        ...
        private void RecogniseFaceOut_Shown(object sender, EventArgs e)
        {
            //Initialize the capture device
            grabber = new Capture(1);
            grabber.QueryFrame();
            //Initialize the FrameGraber event
            Application.Idle += new EventHandler(this.FrameGrabber);
        }

        //LOAD LOAD LOAD LOAD LOAD
        private void RecogniseFaceOut_Load(object sender, EventArgs e)
        {
            timer1.Start();
                ...
                if (trainingImages.ToArray().Length != 0)
                {
                    ...
                    if (DataAccess.IsStudent(name))
                    {
                        ...
                        PositionTextBox.Text = "Student";
                            ...
                            //If IN the allow exit -- for lock/unlock mechanism (no implementation yet)
                    ...
                    else if (DataAccess.IsInstructor(name))
                    {
                        ...
                        //Then Record attendance
                        DataAccess.WriteAttendanceOut(Convert.ToInt32(name), DateTime.Now, DateTime.Now, DateTime.Now, _scheduleId);
                        Unlock();
